...
TEST(test_thread_pool, schedule_simple)
{
    ...
    auto sched = std::make_shared<mock_scheduler>();
    core::thread_pool pool(sched, 1);

    EXPECT_EQ(std::size_t(0), pool.thread_count());

    EXPECT_CALL(*mt, run()).Times(1);
    EXPECT_CALL(*sched, empty()).WillRepeatedly(Return(true));
...
{
    ultra::sched_ptr _sched;
    ultra::core::thread_pool _pool;
    ...
    typed_test_thread_pool()
        : _sched(std::make_shared<Tp>()), _pool(_sched) { }
};
...
{
    TestFixture::_pool.execute_callable(0,
                &typed_test_thread_pool<TypeParam>::test_function_nosleep, this);

    TestFixture::_pool.wait_for_done();
...
TYPED_TEST(typed_test_thread_pool, wait_for_done)
{
    ...
    TestFixture::_pool.wait_for_done(10);
    EXPECT_EQ(std::size_t(runs), TestFixture::_run_count);
}
...
TYPED_TEST(typed_test_thread_pool, thread_recycling)
{
    ...
    TestFixture::_pool.execute(task);
    task->_cond.wait(lk);
...
TYPED_TEST(typed_test_thread_pool, expiry_timeout)
{
    ...
    std::chrono::milliseconds expiry_timeout = TestFixture::_pool.expiry_timeout();
    TestFixture::_pool.set_expiry_timeout(std::chrono::milliseconds(10));
    EXPECT_EQ(std::chrono::milliseconds(10), TestFixture::_pool.expiry_timeout());
    ...
    TestFixture::_pool.set_expiry_timeout(expiry_timeout);
    EXPECT_EQ(expiry_timeout, TestFixture::_pool.expiry_timeout());
}
...
TYPED_TEST(typed_test_thread_pool, timeout_race)
{
    ...
    TestFixture::_pool.set_max_thread_count(1);
    TestFixture::_pool.set_expiry_timeout(std::chrono::milliseconds(50));
...
TEST_P(typed_test_thread_pool_max_thread_count, max_thread_count_invoke)
{
    ...
    EXPECT_CALL(*static_cast<mock_scheduler *>(_sched.get()),
                empty()).WillRepeatedly(Return(true));

    _pool.set_max_thread_count(GetParam());
    EXPECT_EQ(GetParam(), _pool.max_thread_count());
}
...
TYPED_TEST(typed_test_thread_pool, start_stop_threads)
{
    ...
    TestFixture::_pool.set_max_thread_count(2);
    sem_trywait(&task->_sem_for_started);
    ...
    TestFixture::_pool.execute(task);
    TestFixture::_pool.set_max_thread_count(4);
    sem_trywait(&task->_sem_for_started);
    sem_trywait(&task->_sem_for_started);
    ...
    TestFixture::_pool.set_max_thread_count(2);
    EXPECT_EQ(std::size_t(4), TestFixture::_pool.thread_count());
    ...
    TestFixture::_pool.execute(task);
    TestFixture::_pool.execute(task);
    TestFixture::_pool.set_max_thread_count(8);
    sem_trywait(&task->_sem_for_started);
    sem_trywait(&task->_sem_for_started);
    sem_trywait(&task->_sem_for_started);
    sem_trywait(&task->_sem_for_started);
...
TEST_P(typed_test_thread_pool_reserve_thread, reserve_thread)
{
    ...
    for (std::size_t i = 0; i < GetParam(); ++i)
        _pool.reserve_thread();

    _pool.reserve_thread();
    EXPECT_EQ(_pool.thread_count(), (GetParam() > 0 ? GetParam() : 0) + 1);
    _pool.reserve_thread();
    EXPECT_EQ(_pool.thread_count(), (GetParam() > 0 ? GetParam() : 0) + 2);

    _pool.release_thread();
    _pool.release_thread();
...
TEST_P(typed_test_thread_pool_reserve_thread, release_thread)
{
    ...
    std::size_t reserved = _pool.thread_count();
    while(reserved-- > 0) {
        _pool.release_thread();
        EXPECT_EQ(reserved, _pool.thread_count());
    }

    EXPECT_EQ(std::size_t(0), _pool.thread_count());

    _pool.release_thread();
    EXPECT_EQ(std::size_t(-1), _pool.thread_count());
    _pool.reserve_thread();
    EXPECT_EQ(std::size_t(0), _pool.thread_count());
}
...
{
    TestFixture::_pool.set_max_thread_count(1);
    EXPECT_EQ(std::size_t(0), TestFixture::_pool.thread_count());

    TestFixture::_pool.reserve_thread();
    EXPECT_EQ(std::size_t(1), TestFixture::_pool.thread_count());

    std::shared_ptr<waiting_task> task = std::make_shared<waiting_task>();

    TestFixture::_pool.execute(task);
    EXPECT_EQ(std::size_t(2), TestFixture::_pool.thread_count());
    ...
    TestFixture::_pool.release_thread();
    EXPECT_EQ(std::size_t(0), TestFixture::_pool.thread_count());
}
